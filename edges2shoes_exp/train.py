...
def visualize_multi(opt, real_A, model, eidx, uidx):
    ...
    multi_prior_z_B = Variable(real_A.data.new(opt.num_multi,
        opt.nlatent, 1, 1).normal_(0, 1).repeat(size[0],1,1,1), requires_grad=False)
...
def train_model():
    ...
    if opt.numpy_data:
        ...
        dev_cycle = itertools.cycle(AlignedIterator(devA, devB, batch_size=opt.batchSize))
    else:
        print(opt.batchSize)
    ...
    # store best evaluation in result, init maximine for results
    results = {
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(train_dataset):
            ...
            #     # use gpu for visuals
            #     if use_gpu:
            #         dev_real_A_gen = dev_real_A.cuda()
            #         dev_real_B_gen = dev_real_B.cuda()
            #         dev_prior_z_B_gen = dev_prior_z_B.cuda()

            #     dev_visuals = model.generate_cycle(dev_real_A_gen, dev_real_B_gen, dev_prior_z_B_gen)
            ...
            if total_steps % opt.print_freq == 0:
                t = (time.time() - print_start_time) / opt.batchSize # time per image
