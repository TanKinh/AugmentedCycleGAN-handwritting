...
class AugmentedCycleGAN(object):
    ...
    def inference_multi_A(self, real_A, real_B):
        ...
        multi_real_B = real_B.unsqueeze(1).repeat(1, num, 1, 1, 1)
        multi_real_B = multi_real_B.view(size[0]*num,size[1],size[2],size[3])

        if self.opt.enc_A_B:
            fake_B = self.netG_A_B.forward(real_A)
            concat_B_A = torch.cat((fake_B, real_A), 1)
            mu_z_A, logvar_z_A = self.netE_B.forward(concat_B_A)
        else:
            mu_z_A, logvar_z_A = self.netE_B.forward(real_A)

        if self.opt.stoch_enc:
            post_z_A = gauss_reparametrize(mu_z_A, logvar_z_A)
        else:
            post_z_A = mu_z_B.view(mu_z_A.size(0), mu_z_A.size(1), 1, 1)

        multi_post_z_A = Variable(post_z_A.data.repeat(size[0],1,1,1), volatile=True)

        multi_fake_A = self.netG_B_A.forward(multi_real_B, multi_post_z_A)
