...
def eval_mse_A(dataset, model, use_gpu=True):
    ...
    for batch in dataset:
        ...
        mse_A.append(F.mse_loss(pred_A, real_A).data)
        # print(np.mean([x.item() for x in self.stats[l]]))
    return np.mean([x.item() for x in mse_A])
...
def eval_ubo_B(dataset, model, steps=500, visualize=False, vis_name=None, vis_path=None,
    ...
    for batch in dataset:
        ...
        bpp_B.append(bpp)
        kld_B.append(kld)
    return np.mean([x.item() for x in ubo_B]), np.mean([y.item() for y in bpp_B]), np.mean([z.item() for z in kld_B])
...
def variational_ubo(model, real_A, real_B, steps, visualize=False, vis_name=None, vis_path=None,
    ...
    rA = Variable(real_A.data, requires_grad=True)
